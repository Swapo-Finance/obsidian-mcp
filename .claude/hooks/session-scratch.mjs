 ...
 * whose path is inherently unique per session, preventing state collision
 * when multiple sessions of the same project run in parallel (per-project
 * hashing would cause concurrent sessions to collide on the same path).
 *
