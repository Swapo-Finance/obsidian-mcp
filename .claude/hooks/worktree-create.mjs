...
// Placed before any git() call below (real mutations start much further down).
...
// then process.cwd() — ensures each session operates in its correct project root.
...
if (fs.existsSync(path.join(cwd, "package.json"))) {
  ...
  if (fs.existsSync(rootModules)) {
    ...
    // from external input, so this carries none of the injection risk that the
    // "no shell form" convention targets — that convention is about a hook's own
    // top-level settings.json invocation (use exec form: `command` + `args`, so
    // no shell parses a path), which is a different case from this spawn.
