...
from ..constants import ERROR_MESSAGES, MARKDOWN_EXTENSIONS
...
def validate_note_path(path: str) -> tuple[bool, str | None]:
    """
...
def validate_context_length(length: int) -> tuple[bool, str | None]:
    """
...
def validate_date_search_params(
    date_type: str, days_ago: int, operator: str
) -> tuple[bool, str | None]:
    """
    ...
    if days_ago > 365:
        return (
            False,
            f"Days ago too large: {days_ago} (max: 365). Use smaller values or implement year-based search.",
        )
...
def validate_tags(tags: list[str]) -> tuple[bool, str | None]:
    ...
    if len(cleaned_tags) > 50:
        return (
            False,
            f"Too many tags: {len(cleaned_tags)} (max: 50). Consider organizing with folders instead.",
        )

    return True, None


def validate_directory_path(path: str | None) -> tuple[bool, str | None]:
    """
...
def validate_content(content: str) -> tuple[bool, str | None]:
    ...
    if len(content) > 1_000_000:
        return (
            False,
            f"Content too large: {len(content)} characters (max: 1,000,000). Consider splitting into multiple notes.",
        )
