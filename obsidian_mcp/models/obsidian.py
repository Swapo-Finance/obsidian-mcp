...
class NoteMetadata(BaseModel):
    ...
    aliases: list[str] = Field(
        default_factory=list, description="Alternative names for the note"
    )
    frontmatter: dict[str, Any] = Field(
        default_factory=dict, description="Raw frontmatter data"
    )
...
class Note(BaseModel):
    ...
    @classmethod
    def validate_path(cls: type["Note"], v: str) -> str:
...
class VaultItem(BaseModel):
    ...
    children: list["VaultItem"] | None = Field(
        default=None, description="Child items if this is a folder"
    )
...
class SearchResult(BaseModel):
    ...
    matches: list[str] = Field(
        default_factory=list, description="Matching text excerpts"
    )
    context: str | None = Field(default=None, description="Context around the match")
...
class Tag(BaseModel):
    ...
    notes: list[str] = Field(
        default_factory=list, description="Paths to notes with this tag"
    )

    @field_validator("name")
    @classmethod
    def clean_tag_name(cls: type["Tag"], v: str) -> str:
...
class Backlink(BaseModel):
    ...
    context: str | None = Field(
        default=None, description="Text around the link for context"
    )
