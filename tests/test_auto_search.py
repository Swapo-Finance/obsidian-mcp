...
from obsidian_mcp.tools.organization import move_note, rename_note
...
def mock_get_vault(mock_vault):
    ...
    Also patches write_policy.get_vault: move_note/rename_note are now
    wrapped by _serialize_note_writes (Finding 2), whose decorator calls
    get_vault() via write_policy.py's own import to reach vault.write_lock
    -- a separate name binding from note_move.py's/note_rename.py's, so it
    needs its own patch.
    """
    with (
...
async def test_rename_note_auto_search_single_match(mock_get_vault):
    ...
    with patch("obsidian_mcp.tools.search_discovery.search_notes") as mock_search:
        ...
        result = await rename_note("My Note.md", "New Note.md", update_links=False)
...
async def test_move_note_auto_search_single_match(mock_get_vault):
    ...
    # Mock search_notes
    with patch("obsidian_mcp.tools.search_discovery.search_notes") as mock_search:
        mock_search.return_value = {"results": [{"path": "Inbox/Todo.md"}], "count": 1}
        ...
        result = await move_note("Todo.md", "Archive/Todo.md")
        ...
        vault.write_note.assert_called_once_with(
            "Archive/Todo.md", note.content, overwrite=False
        )
...
async def test_move_note_auto_search_with_rename(mock_get_vault):
    ...
    with (
        patch("obsidian_mcp.tools.search_discovery.search_notes") as mock_search,
        patch("obsidian_mcp.tools.link_management.get_backlinks") as mock_backlinks,
    ):
        mock_search.return_value = {
        ...
        result = await move_note(
            "Draft.md", "Projects/Final Document.md", update_links=True
        )
...
async def test_auto_search_preserves_directory_context(mock_get_vault):
    ...
    with patch("obsidian_mcp.tools.search_discovery.search_notes") as mock_search:
        ...
        vault.read_note.side_effect = [FileNotFoundError(), note, FileNotFoundError()]
        vault.write_note.return_value = None
        vault.delete_note.return_value = None

        # Rename with just filename
        result = await rename_note(
            "Sprint Planning.md", "Q1 Retrospective.md", update_links=False
        )
...
async def test_auto_search_with_context_logging(mock_get_vault):
    ...
    note = Note(
        path="Daily/2024-01-15.md", content="Daily note", metadata=NoteMetadata()
    )
    ...
    with patch("obsidian_mcp.tools.search_discovery.search_notes") as mock_search:
        ...
        await rename_note(
            "2024-01-15.md", "2024-01-15-reviewed.md", update_links=False, ctx=ctx
        )

        # Check that helpful messages were logged
        ctx.info.assert_any_call(
            "Note not found at 2024-01-15.md, searching for filename..."
        )
