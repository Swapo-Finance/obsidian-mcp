...
class TestPropertySearch:
    ...
    async def test_search_by_not_equal(self, test_vault):
        ...
        results = await _search_by_property(
            test_vault, "property:status:!=completed", 100
        )
    ...
    async def test_search_notes_with_property_syntax(self, test_vault):
        ...
        with patch(
            "obsidian_mcp.tools.search_discovery.get_vault", return_value=test_vault
        ):
...
class TestPropertyTypes:
    ...
    async def test_date_property(self, test_vault):
        ...
        results = await _search_by_property(
            test_vault, "property:deadline:>2024-06-01", 100
        )
        assert len(results) >= 1
...
class TestEdgeCases:
    ...
    async def test_nonexistent_property(self, test_vault):
        results = await _search_by_property(
            test_vault, "property:nonexistent:value", 100
        )
