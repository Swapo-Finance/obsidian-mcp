...
"""
    ...
    async def test_update_properties(self, test_index):
        # Index a file
        metadata1 = {"frontmatter": {"status": "draft", "priority": 1}}
        await test_index.index_file("note.md", "Content 1", 1000, 50, metadata1)

        # Update with new properties
        metadata2 = {
            "frontmatter": {"status": "published", "priority": 3, "author": "John Doe"}
        }
...
class TestPropertySearch:
    ...
    async def test_search_not_equals(self, test_index):
        # Index test data
        await test_index.index_file(
            "note1.md", "Content 1", 1000, 50, {"frontmatter": {"status": "active"}}
        )
        await test_index.index_file(
            "note2.md", "Content 2", 2000, 60, {"frontmatter": {"status": "inactive"}}
        )
        await test_index.index_file(
            "note3.md", "Content 3", 3000, 70, {"frontmatter": {"title": "No status"}}
        )
    ...
    async def test_search_numeric_comparison(self, test_index):
        ...
        await test_index.index_file(
            "low.md", "Low priority", 1000, 50, {"frontmatter": {"priority": 1}}
        )
        await test_index.index_file(
            "medium.md", "Medium priority", 2000, 60, {"frontmatter": {"priority": 3}}
        )
        await test_index.index_file(
            "high.md", "High priority", 3000, 70, {"frontmatter": {"priority": 5}}
        )
    ...
    async def test_search_contains(self, test_index):
        # Index test data
        await test_index.index_file(
            "john.md", "John's note", 1000, 50, {"frontmatter": {"author": "John Doe"}}
        )
        await test_index.index_file(
            "jane.md",
            "Jane's note",
...
class TestPropertyMetadata:
    ...
    async def test_get_property_values(self, test_index):
        ...
        await test_index.index_file(
            "note2.md", "Content 2", 2000, 60, {"frontmatter": {"status": "active"}}
        )
        await test_index.index_file(
            "note3.md", "Content 3", 3000, 70, {"frontmatter": {"status": "inactive"}}
        )
...
@pytest.mark.asyncio
class TestPropertyCleanupOnRemoval:
    """Finding 1 (code review): remove_file() must also clear
    file_properties. file_properties.filepath declares
    ON DELETE CASCADE, but SQLite disables FK enforcement by default per
    connection, so that cascade never actually fired -- confirmed
    empirically by removing a file with properties and finding its
    file_properties rows still present afterward. Without this, every
    deleted/renamed-away note leaked its property rows forever, and those
    orphaned rows surfaced in get_all_property_names()/get_property_values()
    for notes that no longer exist.
    """

    async def test_remove_file_clears_its_properties(self, test_index):
        await test_index.index_file(
            "note.md",
            "Content",
            1000,
        ...
        assert await test_index.get_all_property_names() == ["priority", "status"]

        await test_index.remove_file("note.md")

        db = test_index._require_db()
        cursor = await db.execute(
            "SELECT * FROM file_properties WHERE filepath = ?", ("note.md",)
        )
        assert await cursor.fetchall() == []

    async def test_removed_file_properties_do_not_leak_into_listings(self, test_index):
        await test_index.index_file(
            "note.md", "Content", 1000, 50, {"frontmatter": {"status": "active"}}
        )

        await test_index.remove_file("note.md")

        assert await test_index.get_all_property_names() == []
        assert await test_index.get_property_values("status") == []

    async def test_remove_file_only_clears_the_removed_files_properties(
        self, test_index
    ):
        await test_index.index_file(
            "note1.md", "Content 1", 1000, 50, {"frontmatter": {"status": "active"}}
        ...
        await test_index.remove_file("note1.md")

        results = await test_index.search_by_property("status", "=", "active")
        assert len(results) == 1
        assert results[0]["filepath"] == "note2.md"
...
class TestComplexProperties:
    ...
    async def test_object_properties(self, test_index):
        ...
        await test_index.index_file(
            ...
            1000,
            50,
            {"frontmatter": {"metadata": {"version": "1.0", "format": "markdown"}}},
        )
