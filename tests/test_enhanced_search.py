...
async def test_enhanced_default_search():
    ...
    mock_vault.get_last_search_metadata = MagicMock(
        return_value={"truncated": False, "total_count": 2}
    )
    ...
    try:
        ...
        assert filename_match is not None, (
            "Should find 'Obsidian Tag Refactor.md' in results"
        )
        ...
        if len(results["results"]) > 1:
            ...
            assert (
                "Tag Refactor" in first_result["path"]
                or "tag refactor" in first_result["path"].lower()
            )
